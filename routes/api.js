...
// NOTE: Authentication middleware (requireAuth) is applied in server.js before mounting this router
// So req.user is always available in these routes
...
// Get sessions for date (admin view with bookings)
router.get('/sessions/date/:date', requireAdmin, async (req, res) => {
  try {
...
router.get('/session/:id/details', requireAdmin, async (req, res) => {
  try {
...
router.post('/session', requireAdmin, async (req, res) => {
  try {
...
router.put('/booking/:id/notes', requireAdmin, async (req, res) => {
  try {
    if (!validator.isMongoId(req.params.id)) {
      return res.status(400).json({ error: 'Invalid booking ID' });
    }
...
router.post('/client/:id/add-package', requireAdmin, async (req, res) => {
  try {
    if (!validator.isMongoId(req.params.id)) {
...
router.post('/client/:id/reset-package', requireAdmin, async (req, res) => {
  try {
    if (!validator.isMongoId(req.params.id)) {
      return res.status(400).json({ error: 'Invalid client ID' });
    }
...
router.post('/send-session-email', requireAdmin, async (req, res) => {
  try {
